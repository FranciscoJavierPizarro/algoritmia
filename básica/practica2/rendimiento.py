...
def _performanceTest3Algs():
    """Compara el coste temporal para N = 100 y P e [1-25] para los tres algoritmos""" 
    N = 20
    with open("rendimientos/rendimientoPyth.txt", "w") as fpy,open("rendimientos/rendimientoHaskSimulado.txt", "w") as fhsS,open("rendimientos/rendimientoHaskDirecto.txt", "w") as fhsD: 
        for P in range(2,15):
            ...
            hstime = subprocess.run(comando + " 0", stdout=subprocess.PIPE, shell=True, universal_newlines=True).stdout.split()[0] 
            fhsS.write(str (P) + " " + hstime+ "\n")# iteracion tiempoEnMs

            hstime = subprocess.run(comando, stdout=subprocess.PIPE, shell=True, universal_newlines=True).stdout.split()[0]
            fhsD.write(str (P) + " " + hstime+ "\n")# iteracion tiempoEnMs

def _performanceTest2AlgsProf():
    """Compara el coste temporal para N = 1000000 y P e [10^1-10^7] para los algoritmos hechos en Haskell""" 
    N = 10000
...
def _performanceTest2AlgsBalls():
    """Compara el coste temporal para P = 1000000 y N e [10^1-10^8] para los algoritmos hechos en Haskell""" 
    P = 100000
...
if __name__ == "__main__":
    print("Ejecutando metricas rendimiento de los 3 algoritmos para N = 100 y P e [1-25]")
    _performanceTest3Algs()
    print("Ejecutando metricas rendimiento para N = 1000000 y P e [10^1-10^7] para los algoritmos hechos en Haskell")
    _performanceTest2AlgsProf()
    print("Ejecutando metricas rendimiento para P = 1000000 y N e [10^1-10^8] para los algoritmos hechos en Haskell")
